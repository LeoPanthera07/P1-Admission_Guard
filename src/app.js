 ...
 * New features: Configure Rules, Export (CSV/JSON), Local Folder Storage,
 *               Audit edit / flag / delete
 */
'use strict';

// ─── DEFAULT CONFIG (used if no saved rules exist) ────────────────────────────
var DEFAULT_CONFIG = {
  schemaVersion:'1.0.0',
  ...
  fields:{
    ...
    dateOfBirth:{label:'Date of Birth',type:'date',ruleType:'soft',order:4,
      ...
      messages:{required:'Date of birth is required.',
                ageMin:'Must be at least 18 years old on Jul 1, 2026.',
                ageMax:'Must be 35 years or younger on Jul 1, 2026.'},
      exceptionAllowed:true,softWarning:'Candidate age is outside the standard range (18–35 years).'},
    ...
    graduationYear:{label:'Graduation Year',type:'number',ruleType:'soft',order:6,
      ...
      messages:{required:'Graduation year is required.',min:'Must be 2015 or later.',max:'Must be 2025 or earlier.'},
      exceptionAllowed:true,softWarning:'Graduation year is outside the accepted range (2015–2025).'},
    percentageOrCgpa:{label:'Percentage / CGPA',type:'percentageCgpa',ruleType:'soft',order:7,
      ...
      messages:{required:'Percentage or CGPA is required.',percentageMin:'Must be ≥ 60%.',
                cgpaMin:'CGPA must be ≥ 6.0.',cgpaMax:'CGPA cannot exceed 10.0.'},
      exceptionAllowed:true,softWarning:'Candidate does not meet the minimum academic score criteria.'},
    screeningTestScore:{label:'Screening Test Score',type:'number',ruleType:'soft',order:8,
      ...
      messages:{required:'Screening test score is required.',min:'Cannot be negative.',
                max:'Cannot exceed 100.',passMark:'Must score at least 40 out of 100.'},
      exceptionAllowed:true,softWarning:'Screening test score is below the passing threshold of 40.'},
...
// ─── Live config (may be overridden by saved rules) ───────────────────────────
...
document.addEventListener('DOMContentLoaded', async function () {
  // Load saved rules override if any
  var savedRules = getSavedRules();
  if (savedRules) try { CONFIG = JSON.parse(JSON.stringify(savedRules)); } catch(e) {}

  // Sync from local folder if one was previously chosen
...
function initStoragePanel() {
  wireOnce('btn-pick-folder', async function() {
    ...
    if (result.ok) {
      ...
      if (subs.length) await _persistToDirNow(subs);
    } else {
...
function renderConfigRules() {
  ...
  // softRuleKeywords
  sysGrid.appendChild(_configRow('Exception Keywords (comma-separated)', 'softRuleKeywords', 'text',
    CONFIG.softRuleKeywords.join(', '),
  ...
  // Per-field rules
  var sorted = Object.keys(CONFIG.fields).map(function(k){
    return { name:k, cfg:CONFIG.fields[k] };
  ...
  sorted.forEach(function(item) {
    ...
    var header = _el('div', 'config-card__header');
    var title  = _el('div', 'config-card__title');
    title.textContent = cfg.label;
    var badgeEl = _el('span', 'rule-badge rule-badge--' + cfg.ruleType);
    badgeEl.textContent = cfg.ruleType.charAt(0).toUpperCase() + cfg.ruleType.slice(1);
    ...
    // Soft-only numeric rule params
    if (fn === 'dateOfBirth') {
      grid.appendChild(_configRow('Min Age', fn+'_ageMin', 'number', v.ageMin,
        function(val){ CONFIG.fields[fn].validations.ageMin = parseFloat(val)||18;
                       CONFIG.fields[fn].messages.ageMin = 'Must be at least '+val+' years old on Jul 1.'; }));
      grid.appendChild(_configRow('Max Age', fn+'_ageMax', 'number', v.ageMax,
        function(val){ CONFIG.fields[fn].validations.ageMax = parseFloat(val)||35;
                       CONFIG.fields[fn].messages.ageMax = 'Must be '+val+' years or younger on Jul 1.'; }));
    }
    if (fn === 'graduationYear') {
      grid.appendChild(_configRow('Min Year', fn+'_min', 'number', v.min,
        function(val){ CONFIG.fields[fn].validations.min = parseInt(val,10)||2015;
                       CONFIG.fields[fn].messages.min = 'Must be '+val+' or later.'; }));
      grid.appendChild(_configRow('Max Year', fn+'_max', 'number', v.max,
        function(val){ CONFIG.fields[fn].validations.max = parseInt(val,10)||2025;
                       CONFIG.fields[fn].messages.max = 'Must be '+val+' or earlier.'; }));
    }
    if (fn === 'percentageOrCgpa') {
      grid.appendChild(_configRow('Min Percentage (%)', fn+'_pct', 'number', v.percentageMin,
        function(val){ CONFIG.fields[fn].validations.percentageMin = parseFloat(val)||60;
                       CONFIG.fields[fn].messages.percentageMin = 'Must be ≥ '+val+'%.'; }));
      grid.appendChild(_configRow('Min CGPA (/10)', fn+'_cgpa', 'number', v.cgpaMin,
        function(val){ CONFIG.fields[fn].validations.cgpaMin = parseFloat(val)||6;
                       CONFIG.fields[fn].messages.cgpaMin = 'CGPA must be ≥ '+val+'.'; }));
    }
    if (fn === 'screeningTestScore') {
      grid.appendChild(_configRow('Pass Mark', fn+'_pass', 'number', v.passMark,
        function(val){ CONFIG.fields[fn].validations.passMark = parseFloat(val)||40;
                       CONFIG.fields[fn].messages.passMark = 'Must score at least '+val+' out of 100.'; }));
    }
    if (fn === 'highestQualification') {
      grid.appendChild(_configRow('Allowed Qualifications (comma-separated)', fn+'_enum', 'text',
        (v.enum||[]).join(', '),
        function(val){ CONFIG.fields[fn].validations.enum = val.split(',').map(function(k){ return k.trim(); }).filter(Boolean); }));
    }

    // Soft warning message (for soft fields)
    if (cfg.ruleType === 'soft' && cfg.softWarning !== undefined) {
      grid.appendChild(_configRow('Soft Warning Message', fn+'_warn', 'text', cfg.softWarning,
        function(val){ CONFIG.fields[fn].softWarning = val; }));
    }

    if (grid.children.length === 0) {
      var note = _el('p','config-no-params');
      note.textContent = 'No configurable parameters for this field (strict/fixed rule).';
  ...
  if (actions) {
    wireOnce('btn-save-rules', function() {
      saveRules(CONFIG);
      showToast('Rules saved! New entries will use updated rules.', 'success');
    });
    ...
    wireOnce('btn-export-rules', function() {
      ...
      a.href = url; a.download = 'admitguard-rules.json';
...
function _configRow(label, id, type, defaultVal, onChange) {
  ...
  inp.id    = 'cfg-' + id;
  inp.value = defaultVal !== undefined ? defaultVal : '';
...
function renderForm(prefill) {
  ...
  // Show editing banner if prefill
...
function buildFieldGroup(fieldName, cfg) {
  ...
  if (cfg.validations.required) {
    var req = _el('span','required-mark'); req.textContent = ' *';
...
function buildSoftArea(fieldName, cfg) {
  ...
  chkLabel.textContent = 'Request Exception';
  ...
  ta.placeholder = 'Min 30 chars, include: "approved by", "special case", "documentation pending", or "waiver granted"';
  ta.rows = 3;
  ...
  rationaleDiv.appendChild(ta); rationaleDiv.appendChild(charCount); rationaleDiv.appendChild(rationaleErr);
  area.appendChild(rationaleDiv);

  chk.addEventListener('change', function(){
    EXCEPTIONS[fieldName].used = chk.checked;
    rationaleDiv.style.display = chk.checked ? 'flex' : 'none';
    if (chk.checked){ rationaleDiv.style.flexDirection='column'; rationaleDiv.style.gap='4px'; }
    else {
...
function syncSubmitButton() {
  ...
  if (!res.isFormValid || isRejected) { btn.disabled=true; btn.setAttribute('aria-disabled','true'); return; }
  ...
  btn.disabled = softBlocked; btn.setAttribute('aria-disabled', softBlocked?'true':'false');
}
...
function showSuccessScreen(record) {
  ...
  var flagHtml = record.flaggedForManager
    ? '<div class="success-flag-banner">⚠ Flagged for manager review — '+record.exceptionCount+' exception(s)</div>'
    : '';
  ...
  wireOnce('new-candidate-btn', function(){
    ...
    resetForm();
    var fv2 = document.getElementById('view-form'); if (fv2) fv2.classList.add('active');
...
// ═══════════════════════════════════════════════════════════════════════════════
// AUDIT LOG — with Edit, Flag/Unflag, Delete
// ═══════════════════════════════════════════════════════════════════════════════
...
function renderAuditLog() {
  ...
  tbody.innerHTML = filtered.map(function(s, i) {
    ...
    return '<tr class="'+(s.flaggedForManager?'flagged-row':'')+'" data-id="'+escHtml(s.id)+'">' +
      ...
      '<td class="audit-actions">' +
        '<button class="btn-audit btn-audit--edit"   data-id="'+escHtml(s.id)+'" title="Edit this entry">✏️ Edit</button>' +
        '<button class="'+flagBtnCls+'" data-id="'+escHtml(s.id)+'" title="Toggle flag">🚩 '+flagBtnTxt+'</button>' +
        '<button class="btn-audit btn-audit--delete" data-id="'+escHtml(s.id)+'" title="Delete this entry">🗑 Delete</button>' +
      '</td>' +
    '</tr>';
  }).join('');
  ...
  tbody.querySelectorAll('.btn-audit--flag').forEach(function(btn){
    btn.addEventListener('click', async function(e){
      e.stopPropagation();
      var id = btn.getAttribute('data-id');
      var updated = await toggleFlag(id);
      SUBS = getSubmissions();
  ...
  tbody.querySelectorAll('.btn-audit--unflag').forEach(function(btn){
    btn.addEventListener('click', async function(e){
      e.stopPropagation();
      var id = btn.getAttribute('data-id');
      var updated = await toggleFlag(id);
  ...
  tbody.querySelectorAll('.btn-audit--delete').forEach(function(btn){
    btn.addEventListener('click', async function(e){
      ...
      var id   = btn.getAttribute('data-id');
      var subs = getSubmissions();
      var rec  = subs.find(function(s){ return s.id===id; });
      if (!rec) return;
...
function editEntry(record) {
  // Switch to form view with record pre-filled
  showView('form');
  renderForm(record);
